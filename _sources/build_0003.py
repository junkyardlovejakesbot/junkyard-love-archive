#!/usr/bin/env python3
"""Process Junkyard Love episode 0003 — Spencer Hicks (single guest, EXISTING APPEND).
YT=RSS titles match exactly ("Episode 003 with Spencer Hicks").
About YT~RSS near-identical (RSS: "sit down"/"In this chat"; YT: "chat"/"In this sit-down");
prefer RSS Jacob copy; YT footer stripped. Inventory has_quotes=no, has_timestamps=no,
has_hashtags=no, has_guest_links=no.
Guest slug: spencer-hicks (EXISTING — lists 0088+0074+0059+0044+0038+0023+0017+0011; APPEND 0003, do not wipe).
YouTube auto-captions available (en/en-orig). YT duration ~4813s vs RSS 4786s (~27s —
not substantial; meta uses RSS). YouTube chapter markers present — Archive picks uses YT chapters.
Archive picks required. Imperfect auto diarization flagged. No guest contact/email.
Insert indexes after 0004. After 0003, next catalog is 0002 Rob Gonzalez of EYE Clothing
(check carefully vs existing roberto-gonzalez from 0032 before creating a new guest — do not build here)."""
...
CONTENT = ROOT / 'content/0003-spencer-hicks'
SITE = ROOT / 'site'
DEPLOY = Path('/workspace/junkyard-love-archive-deploy')
EP_SLUG = '0003-spencer-hicks'
GUEST_SLUG = 'spencer-hicks'  # EXISTING — APPEND
TITLE = 'Episode 003 with Spencer Hicks'
RSS_TITLE = TITLE
YT_TITLE = TITLE
GUEST = 'Spencer Hicks'
GUEST_SHORT = 'Spencer'
...
SPK = 'Spencer'
YOUTUBE = 'https://www.youtube.com/watch?v=xBXbCjWmYcA'
RSS_URL = 'https://share.transistor.fm/s/75ec3d31'
AUDIO_URL = 'https://2.gum.fm/op3.dev/e/pdcn.co/e/pscrb.fm/rss/p/pdst.fm/e/dts.podtrac.com/redirect.mp3/media.transistor.fm/75ec3d31/34495f07.mp3'
DATE = '2019-11-25'
DURATION_S = 4786  # RSS / inventory (~1:19:46)
YT_DURATION_S = 4813  # YT info.json (~1:20:13); ~27s delta — not substantial
EP_NUM = '0003'
EP_INT = 3
...
soft_breaks = [
    'In this chat, Spencer and I',
    'An amateur science boy',
]
...
(CONTENT / 'guest-share-draft.txt').write_text(
    ...
    f'Title conflict: {TITLE_CONFLICT}\nGuest slug: {GUEST_SLUG} (EXISTING — APPEND 0003 to 0088+0074+0059+0044+0038+0023+0017+0011, do not wipe)\n'
...
def score_spencer(text):
    ...
    s = 0
    for pat in [
        r'\bego\b', r'\bconsciousness\b', r'\bmindful(?:ness)?\b', r'\bmeditation\b',
        r'\bdeterminism\b', r'\badhd\b', r'\bsam harris\b', r'\bthird person\b',
        r'\bskeptic(?:al|ism)?\b', r'\bhumaniz', r'\bintention\b', r'\bpresent moment\b',
        r'\bposture\b', r'\bclients?\b', r'\bfitness\b', r'\bgym\b',
        r'\bcalories?\b', r'\bworkout\b', r'\bvegan\b', r'\bdiet\b',
        r'\bworldview\b', r'\bsocial media\b', r'\binstagram\b',
        ...
        r'\bmy (?:clients?|posture|fitness|ego)\b',
...
for i, (ms, text) in enumerate(atoms):
    ...
    if ms < 200000 and re.search(r'junkyard love|welcome to the junk|spencer hicks welcome|we are in for the junkyard|my friend spencer|welcome aboard|got spencer on again|episode (?:003|3)', text.lower()):
        sp = 'Jacob'
    elif sj > sg + 1:
        sp = 'Jacob'
    elif sg > sj + 1:
...
keywords = (
    'Spencer Hicks, Episode 003 with Spencer Hicks, Junkyard Love Podcast episode 0003, JYLP 0003, '
    'ego, consciousness, mindfulness, meditation, ADHD, determinism, Sam Harris, third-person perspective, '
    'skepticism, humanizing people, intention, present moment, amateur science boy, armchair philosopher, '
    'Jacob Rhines'
)
hashtags = (
    '#SpencerHicks #JYLP0003 #JunkyardLove #Ego #Consciousness #Mindfulness '
    '#Meditation #ADHD #Determinism #Conversation #JYLP'
)
guest_bio = (
    'Spencer Hicks appears on Junkyard Love episode 0003 as Jacob\'s friend — an amateur science boy and '
    'armchair philosopher whose conversations push Jacob\'s thinking. This early chat dances around ego, '
    'mindfulness, and consciousness, with YouTube chapter markers covering changing your ego, third-person '
    'perspective, skepticism, humanizing people, determinism, words/intent, meditation, conversation, and ADHD. '
    'Inventory has_quotes=no / has_timestamps=no / has_hashtags=no / has_guest_links=no. Guest slug spencer-hicks '
    'EXISTING APPEND (0088+0074+0059+0044+0038+0023+0017+0011+0003). No guest contact/email.'
)
...
(CONTENT / 'TRANSCRIPT_SOURCE.txt').write_text(
    ...
    f"Guest name spelling: Spencer Hicks (YT/RSS titles / inventory); slug {GUEST_SLUG} (EXISTING — APPEND 0003 to 0088+0074+0059+0044+0038+0023+0017+0011, do not wipe).\n"
...
(CONTENT / 'SOURCES.txt').write_text(
    ...
    f"Title source: YT=RSS exact match ({VIDEO_ID}). H1/slug use Episode 003 with Spencer Hicks. {TITLE_CONFLICT}.\n"
    f"Title: {TITLE}\n"
    ...
    f"Guest name spelling: Spencer Hicks; guest slug {GUEST_SLUG} (EXISTING — APPEND 0003; prior 0088+0074+0059+0044+0038+0023+0017+0011 preserved)\n"
...
# Guest page EXISTS — surgical APPEND 0003 (do not wipe prior listings/About blocks)
...
if EP_SLUG in old and f'Episode {EP_NUM}' in old and '0003-spencer-hicks' in old:
    # already appended — rewrite carefully by re-reading and ensuring count
    pass

# Update episode count 8 -> 9
old2 = re.sub(
    r'Appeared on The Junkyard Love Podcast \((\d+) episodes?\)',
    'Appeared on The Junkyard Love Podcast (9 episodes)',
    old,
    count=1,
)
...
# Insert after 0011 list item (descending chrono)
marker_li = (
    '  <li><a href="../../episodes/0011-spencer-hicks/index.html">Episode 011 with Spencer Hicks</a>'
    '<br><span class="note">2019-12-13 · Episode 0011</span></li>\n'
)
if EP_SLUG not in old2:
    if marker_li not in old2:
        raise SystemExit('guest page 0011 list marker not found')
    old2 = old2.replace(marker_li, marker_li + new_li)

# Insert About block for 0003 before Guest links (after 0011 about section)
about_block = (
...
if f'<h3>Episode {EP_NUM}</h3>' not in old2:
    ...
    # refresh About block contents for 0003 without touching others
    old2 = re.sub(
...
# Insert AFTER 0004 (descending: 0004 then 0003)
marker_home = (
    '  <li><a href="episodes/0004-zack-wyld/index.html">'
    'Episode 004 with Zack Wyld of Wyld Productions</a>'
    '<br><span class="note">2019-11-25 · Zack Wyld of Wyld Productions · 1:27:03</span></li>\n'
)
marker_ep = (
    '  <li><a href="episodes/0004-zack-wyld/index.html">'
    'Episode 004 with Zack Wyld of Wyld Productions</a>'
    '<br><span class="note">Episode 0004 · 2019-11-25 · 1:27:03 · Guest: Zack Wyld of Wyld Productions</span></li>\n'
)

home = (SITE / 'index.html').read_text(encoding='utf-8')
if EP_SLUG not in home:
    if marker_home not in home:
        raise SystemExit('home 0004 marker not found')
...
if EP_SLUG not in ep_index:
    if marker_ep not in ep_index:
        raise SystemExit('ep index 0004 marker not found')
...
if EP_SLUG not in sm:
    ...
    m12 = f'  <url><loc>{SITE_BASE}/episodes/0004-zack-wyld/episode.md</loc></url>\n'
    if m12 not in sm:
        raise SystemExit('sitemap 0004 marker not found')
...
if EP_SLUG not in llms:
    ...
    m12_line = (
        f'- [0004 Zack Wyld]'
        f'({SITE_BASE}/episodes/0004-zack-wyld/) — 2019-11-25\n'
    )
    if m12_line not in llms:
        raise SystemExit('llms 0004 ep line not found')
    ...
    m12_md = (
        f'- [{SITE_BASE}/episodes/0004-zack-wyld/episode.md]'
        f'({SITE_BASE}/episodes/0004-zack-wyld/episode.md)\n'
    )
...
if EP_SLUG not in readme and '**0003**' not in readme:
    marker = '- **0004** Zack Wyld of Wyld Productions — `site/episodes/0004-zack-wyld/`\n'
    add = f'- **0003** Spencer Hicks — `site/episodes/{EP_SLUG}/`\n'
    if marker not in readme:
        raise SystemExit('README 0004 marker not found')
...
if DEPLOY.exists():
    ...
    if (DEPLOY / '_sources').exists():
        shutil.copy(ROOT / 'build_0003.py', DEPLOY / '_sources' / 'build_0003.py')
    print('deploy synced (no commit/push)')

print('DONE')
print('slug', EP_SLUG)
print('guest', GUEST_SLUG, 'EXISTING APPEND — episodes after update: 0088, 0074, 0059, 0044, 0038, 0023, 0017, 0011, 0003')
gcheck = (SITE / 'guests' / GUEST_SLUG / 'index.html').read_text(encoding='utf-8')
print('guest has 0003', EP_SLUG in gcheck)
print('guest note episodes:', re.search(r'\((\d+) episodes?\)', gcheck).group(1) if re.search(r'\((\d+) episodes?\)', gcheck) else '?')
for ep in ['0088', '0074', '0059', '0044', '0038', '0023', '0017', '0011', '0003']:
