...
"""Transcribe 0047 MP3 with faster-whisper small/int8; emit VTT + json3 + segments."""
from __future__ import annotations
import json, sys, time
from pathlib import Path

ROOT = Path('/workspace/junkyard-love-archive/content/0047-trenten-kesler-searching-for-fluidity-learning-from-detail-and-laughing-with-everyone')
...
def main():
    ...
    with LOG.open('w') as log:
        ...
        OUT_VTT.write_text('\n'.join(vtt_lines) + '\n', encoding='utf-8')
        # mirrors for archive convention
        ...
        say(f'DONE model={MODEL_NAME}/{COMPUTE} segments={n} words≈{sum(len(s["text"].split()) for s in segs)} elapsed={time.time()-t0:.0f}s')
