...
"""Transcribe 0025 audio with faster-whisper small/int8; emit VTT + json3 + segments.
YouTube has no captions for BRKmGl5XGVA — ASR fallback (upgrade from prior tiny/int8)."""
from __future__ import annotations
import json, time
from pathlib import Path

ROOT = Path('/workspace/junkyard-love-archive/content/0025-jordenelle-tsugawa')
AUDIO = ROOT / 'source-audio.webm'
OUT_JSON = ROOT / 'whisper-segments.json'
OUT_VTT = ROOT / 'source-yt.en.vtt'
OUT_JSON3 = ROOT / 'source-yt.en.json3'
...
def main():
    ...
    with LOG.open('w') as log:
        ...
        OUT_VTT.write_text('\n'.join(vtt_lines) + '\n', encoding='utf-8')
        (ROOT / 'source-yt.en-orig.vtt').write_text(OUT_VTT.read_text(encoding='utf-8'), encoding='utf-8')
        (ROOT / 'source-yt.en-orig.json3').write_text(OUT_JSON3.read_text(encoding='utf-8'), encoding='utf-8')
        # mirror captions.* used by build
        for name in ('captions.en.vtt', 'captions.en-orig.vtt'):
            (ROOT / name).write_text(OUT_VTT.read_text(encoding='utf-8'), encoding='utf-8')
        for name in ('captions.en.json3', 'captions.en-orig.json3'):
            (ROOT / name).write_text(OUT_JSON3.read_text(encoding='utf-8'), encoding='utf-8')
        caps = ROOT / 'captions'
        caps.mkdir(exist_ok=True)
        for name in ('captions.en.vtt', 'captions.en-orig.vtt', 'auto.en.vtt', 'source-yt.en.vtt', 'source-yt.en-orig.vtt'):
            (caps / name).write_text(OUT_VTT.read_text(encoding='utf-8'), encoding='utf-8')
        for name in ('captions.en.json3', 'captions.en-orig.json3', 'auto.en.json3', 'source-yt.en.json3', 'source-yt.en-orig.json3'):
            (caps / name).write_text(OUT_JSON3.read_text(encoding='utf-8'), encoding='utf-8')
